...
def _build_scoring_system_prompt() -> str:
    ...
    return (
        ...
        "【重要原则 — 必须严格遵守】\n"
        "- ⛔ 禁止给任何政策全0分！每条政策的每个维度都必须独立评估，给出合理分数\n"
        "- 金额未知时，score_amount 根据政策类型估算：税收优惠类给60分、认定奖励类给40分、落户/资质类给20分，不要给0\n"
        "- 可得性低不是淘汰理由，而是改进建议方向\n"
        "- 时效性是硬标准：过期政策在紧迫性维度体现，但其他维度仍正常评分\n"
        "- 即使摘要不完整，也要根据标题和已知信息尽力评分\n\n"
        ...
        "- ⚠️ 金额未知时不要给score_amount=0！按政策类型估算：\n"
        "  税收优惠/减免类→60分(B)  认定奖励类→40分(C)  落户/资质/荣誉类→20分(D)  纯信息类→20分(D)\n\n"
...
def _build_round_review_system_prompt() -> str:
    ...
    return (
        ...
        "B. 政策分组覆盖（5组）：产业专项/税收优惠/资质认定/人才激励/用工补贴 各有多少有效结果？\n\n"
        "【判断标准】\n"
        ...
        "  }},\n"
        "  \"layer_coverage\": {{\n"
        "    \"产业专项\": {{\"status\": \"sufficient|insufficient|missing\", \"count\": 0, \"note\": \"...\"}},\n"
        "    \"税收优惠\": {{\"status\": \"sufficient|insufficient|missing\", \"count\": 0, \"note\": \"...\"}},\n"
        "    \"资质认定\": {{\"status\": \"sufficient|insufficient|missing\", \"count\": 0, \"note\": \"...\"}},\n"
        "    \"人才激励\": {{\"status\": \"sufficient|insufficient|missing\", \"count\": 0, \"note\": \"...\"}},\n"
        "    \"用工补贴\": {{\"status\": \"sufficient|insufficient|missing\", \"count\": 0, \"note\": \"...\"}}\n"
        "  }},\n"
        "  \"timeliness\": {{\n"
        "    \"status\": \"good|poor\",\n"
